         ...
         If (!(Test-Path -Path $DestinationPath)) {New-Item -ItemType Directory -Path $DestinationPath | Out-Null} 
                ...
                ForEach($File in $Files) 
                {
					If ($File.extension -eq ".java") {
						New-Item ($File.FullName -replace [regex]::Escape($SourcePath), $DestinationPath -replace "\"+[regex]::Escape($File.Name+$File.Extension), "") -type directory
						Write-Host "Read and Convert $($File.Name)" -ForegroundColor Cyan 
						Get-Content $File.FullName  | Set-Content -Encoding $Encoding ($File.FullName -replace [regex]::Escape($SourcePath), $DestinationPath) -Force -Confirm:$false 
					}
                } 
